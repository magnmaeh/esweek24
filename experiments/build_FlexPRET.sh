...
LF_FP_ROOT=$PROJECT_ROOT/lf-flexpret
...
cp "$LF_FP_ROOT/platform/lf_flexpret_support.c" "$LF_SOURCE_GEN_DIRECTORY/core/platform/"

# Copy header files into /include.
cp "$LF_FP_ROOT/platform/lf_flexpret_support.h" "$LF_SOURCE_GEN_DIRECTORY/include/core/platform/"
cp "$LF_FP_ROOT/platform/platform.h" "$LF_SOURCE_GEN_DIRECTORY/include/core/"
...
APP_INCS := -I$(LF_FP_ROOT)/flexpret/programs/lib/include \
...
' "$LF_FP_ROOT" "$LF_SOURCE_GEN_DIRECTORY" "$LF_FILENAME" '$(THREADS)-1' > "$LF_SOURCE_GEN_DIRECTORY/Makefile"
